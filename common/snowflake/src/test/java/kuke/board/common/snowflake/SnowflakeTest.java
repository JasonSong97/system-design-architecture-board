...
class SnowflakeTest {
	...
	void nextIdTest() throws ExecutionException, InterruptedException {
		...
		for (Future<List<Long>> future : futures) {
			...
			for (int i = 1; i < idList.size(); i++) {
				assertThat(idList.get(i)).isGreaterThan(idList.get(i - 1)); // 오름차순 체크
			}
			result.addAll(idList);
		}
		assertThat(result.stream().distinct().count()).isEqualTo(repeatCount * idCount); // 100만개 확인
	...
	@Test
	@DisplayName("100만개 생성에 걸리는 시간 측정")
